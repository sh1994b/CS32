...
{
	if (n1 < n2)     //examined all a1 up to point where its length is same as a2
		return 0;

	if (n2 == 0)        //empty sequence appears in one way
		return 1;

	if (a1[n1 - 1] == a2[n2 - 1])       //check last elements of both arrays. If equal,
		return countIncludes(a1, n1 - 1, a2, n2 - 1) + countIncludes(a1, n1 - 1, a2, n2);
	//return # of times a2 appeared in the rest of a1 (except last element)
	//plus # times the rest of a2 appeared in the rest of a1

	else                                            //if last elements not equal
		return countIncludes(a1, n1 - 1, a2, n2);   //call function until you find two equal last elements
...
void order(double a[], int n)
{
	...
	{
		split(a, n, splitter, firstNotGreater, firstLess);		//split in half
		if (firstNotGreater != n)			//if there exists an item before the splitter
			order(a, firstNotGreater);		//order first half
		if (firstLess != n)					//if there exists an item after the splitter
